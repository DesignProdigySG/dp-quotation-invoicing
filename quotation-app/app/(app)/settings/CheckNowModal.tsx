...
import {
  listUnprocessedGmailCandidates,
  processSelectedGmailMessages,
...
export default function CheckNowModal({
  connectionEmail,
  onClose,
}: {
  ...
  const [result, setResult] = useState<ProcessSelectedResult | null>(null);

  useEffect(() => {
    let cancelled = false;
    listUnprocessedGmailCandidates().then((res) => {
      if (cancelled) return;
  ...
  async function handleSendToReview() {
    ...
    setError(null);
    try {
      const res = await processSelectedGmailMessages([...selected]);
