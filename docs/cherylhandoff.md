...
1. ~~`clients.default_management_fee_rate`~~ — **done, pushed 2026-08-06**,
   ...
   **Not yet verified**: whether this — the first real (non-placeholder)
   migration ever git-tracked in this repo — actually auto-deployed to the
   `staging` Supabase branch via the Decision 28 Git-connected branching
   pipeline. That pipeline has never been exercised end-to-end before this.
   Check Supabase dashboard → this project → Branches → `staging` →
   migration history for `20260806145155_add_client_management_fee_rate`.
...
- First real migration ever git-tracked in this repo — the Decision 28
  staging pipeline has never been proven end-to-end before this. Verify
  auto-deploy before trusting it for anything bigger.
...
1. Verify Tier 1 piece 1's migration actually reached the `staging` branch
   (blocked on Supabase MCP access in-session — check the dashboard
   directly if needed).
